...
contract Staking is ISigsVerifier, Pauser, Whitelist {
    ...
    event ValidatorStatusUpdate(
        address indexed valAddr,
        dt.ValidatorStatus indexed status
    );
    ...
    function initializeValidator(
        ...
        require(
            validator.status == dt.ValidatorStatus.Null,
            "Validator is initialized"
        );
        require(
            validators[_signer].status == dt.ValidatorStatus.Null,
        ...
        require(
            _commissionRate <= dt.COMMISSION_RATE_BASE,
            "Invalid commission rate"
        );
        require(
            _minSelfDelegation >= params[dt.ParamName.MinSelfDelegation],
        ...
        emit ValidatorNotice(
            valAddr,
            "init",
            abi.encode(_signer, _minSelfDelegation, _commissionRate),
            address(0)
        );
    }
    ...
    function bondValidator() external {
        ...
        nextBondBlock =
            block.number +
            params[dt.ParamName.ValidatorBondInterval];
    ...
    function confirmUnbondedValidator(address _valAddr) external {
        ...
        require(
            validator.status == dt.ValidatorStatus.Unbonding,
            "Validator not unbonding"
        );
        require(
            block.number >= validator.unbondBlock,
    ...
    function completeUndelegate(address _valAddr) external {
        ...
        for (
            i = delegator.undelegations.head;
            i < delegator.undelegations.tail;
            i++
        ) {
            if (
                isUnbonded ||
                delegator.undelegations.queue[i].creationBlock +
                    unbondingPeriod <=
                block.number
            ) {
    ...
    function updateMinSelfDelegation(uint256 _minSelfDelegation) external {
        ...
        if (_minSelfDelegation < validator.minSelfDelegation) {
            ...
            validator.bondBlock = uint64(
                block.number + params[dt.ParamName.AdvanceNoticePeriod]
            );
        }
        validator.minSelfDelegation = _minSelfDelegation;
        emit ValidatorNotice(
     ...
     */
    function slash(
        bytes calldata _slashRequest,
        bytes[] calldata _sigs
    ) external whenNotPaused {
        bytes32 domain = keccak256(
            abi.encodePacked(block.chainid, address(this), "Slash")
        );
        ...
        require(
            request.slashFactor <= dt.SLASH_FACTOR_DECIMAL,
            "Invalid slash factor"
        );
        require(
            request.slashFactor <= params[dt.ParamName.MaxSlashFactor],
            "Exceed max slash factor"
        );
        ...
        uint256 slashAmt = (validator.tokens * request.slashFactor) /
            dt.SLASH_FACTOR_DECIMAL;
        validator.tokens -= slashAmt;
        if (validator.status == dt.ValidatorStatus.Bonded) {
            bondedTokens -= slashAmt;
            if (
                request.jailPeriod > 0 || !hasMinRequiredTokens(valAddr, true)
            ) {
        ...
        uint256 slashUndelegation = (validator.undelegationTokens *
            request.slashFactor) / dt.SLASH_FACTOR_DECIMAL;
    ...
    function getValidatorStatus(
        address _valAddr
    ) public view returns (dt.ValidatorStatus) {
    ...
    function getBondedValidatorsTokens()
        public
        view
        returns (dt.ValidatorTokens[] memory)
    {
        dt.ValidatorTokens[] memory infos = new dt.ValidatorTokens[](
            bondedValAddrs.length
        );
     ...
     */
    function hasMinRequiredTokens(
        address _valAddr,
        bool _checkSelfDelegation
    ) public view returns (bool) {
        ...
        if (_checkSelfDelegation) {
            uint256 selfDelegation = _shareToToken(
                v.delegators[_valAddr].shares,
                valTokens,
                v.shares
            );
    ...
    function getDelegatorInfo(
        address _valAddr,
        address _delAddr
    ) public view returns (dt.DelegatorInfo memory) {
        dt.Validator storage validator = validators[_valAddr];
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (
                isUnbonded ||
                undelegations[i].creationBlock + unbondingPeriod <= block.number
            ) {
    ...
    function _undelegate(
        ...
        require(
            delegator.shares == 0 || delegator.shares >= dt.CELR_DECIMAL,
        ...
        require(
            delegator.undelegations.tail - delegator.undelegations.head <
                dt.MAX_UNDELEGATION_ENTRIES,
            "Exceed max undelegation entries"
        );

        uint256 undelegationShares = _tokenToShare(
            _tokens,
            validator.undelegationTokens,
            validator.undelegationShares
        );
        ...
        dt.Undelegation storage undelegation = delegator.undelegations.queue[
    ...
    function _setUnbondingValidator(address _valAddr) private {
        dt.Validator storage validator = validators[_valAddr];
        validator.status = dt.ValidatorStatus.Unbonding;
        validator.unbondBlock = uint64(
            block.number + params[dt.ParamName.UnbondingPeriod]
        );
        bondedTokens -= validator.tokens;
    ...
    function _decentralizationCheck(uint256 _valTokens) private view {
        ...
        if (bondedValNum == 2 || bondedValNum == 3) {
            require(
                _valTokens < getQuorumTokens(),
                "Single validator should not have quorum tokens"
            );
        } else if (bondedValNum > 3) {
            require(
                _valTokens < bondedTokens / 3,
                "Single validator should not have 1/3 tokens"
            );
        }
    }
